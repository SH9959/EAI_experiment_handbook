...
> **验证范围（2026-09-19）**：本页是待实机复测的修订稿。配套脚本的离线测试、完整网站构建及检查结果见[本次检查记录](../assets/ch3-dialogue/verification.md)。尚未完成真实 API 请求、模型下载与推理、麦克风和摄像头测试。下面的成功判据是学生运行后的验收要求，不是本次已完成的结果。
